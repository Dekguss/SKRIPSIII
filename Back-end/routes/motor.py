...
def search():
    ...
    # Jika tidak ada motor yang ditemukan, kembalikan list kosong
    if not filtered_motors:
        return jsonify([])

    # Kumpulkan semua harga dari motor yang telah difilter
    all_prices = [motor.harga for motor in filtered_motors]
    ...
    for motor in filtered_motors:
        # Gunakan min dan max harga dari hasil filter
        harga_score = calculate_fuzzy_score(
            motor.harga,
            global_min_price,
            global_max_price
        )

        # print(f"Harga Minimal yang digunakan: Rp {global_min_price:,}")
        # print(f"Harga Maksimal yang digunakan: Rp {global_max_price:,}")
